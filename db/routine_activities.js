...
async function getRoutineActivitiesByRoutine({ id }) {
  try{
    const {rows:routineActivity} = await client.query(`
    SELECT * FROM routine_activities WHERE "routineId"=${id}`);
...
async function updateRoutineActivity({ id, ...fields }) {
  try{
    ...
    UPDATE routine_activities 
    SET duration = $1
    WHERE id=$2 RETURNING *;`,[duration,id]);
      returnValue = updatedRoutineActivity
    }
...
try{
  ...
  DELETE FROM
  routine_activities WHERE id=$1 RETURNING *;`,[id]);
...
async function canEditRoutineActivity(routineActivityId, userId) {
  try{
    ...
    SELECT "routineId" FROM routine_activities WHERE id=$1;`,[routineActivityId]);
    console.log(routineId.routineId)
    const {rows:[routineCreatorId]} = await client.query(`
    SELECT "creatorId" FROM routines WHERE id=$1;`,[routineId.routineId]);
